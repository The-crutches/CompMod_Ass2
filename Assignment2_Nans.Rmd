...
#library(rstan)
pm0 <- pp_check(AI_m0_prior, nsamples=100)
...
pm1 <- pp_check(AI_m1_prior, nsamples=100)
...
p4<- pp_check(AI_m1_v2, nsamples=100)
p3 + p4
```
...
p2<-pp_check(AI_m0, nsamples=100)
p1 + p2

p5 <- pp_check(AI_m1_prior, nsamples =100)
p6 <- pp_check(AI_m1_v2, nsamples =100)
p5+p6
...
```{r}
## Plotting model 
conditional_effects(AI_m1_v2, spaghetti = T, nsamples = 100, points = T)
...
#hypothesis(AI_m1, "Diagnosis > 0") #HVor er den her model?
```
...
#Decsribtion from above - 6 variables: 
1) ID, 
...
#Remember only on patients
```{r}
df <- read_csv("Ass2.csv")

df_1 <- filter(df, Diagnosis == 1)
...
df_1 <- select(df_1, c(ID, Diagnosis, AI, VH, MR, AP))
```
...
AIcor <-cor(df_1)
corrplot(AIcor, type="upper", order="original",
